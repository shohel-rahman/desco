...
<html>
  <head>   
	...
	<?php
		...
		$parsed = oci_parse($conn,$yearSql);
		oci_execute($parsed);
		$resultYear = oci_fetch_array($parsed, OCI_BOTH + OCI_RETURN_NULLS);
		$year = $resultYear['YEAR'];
		oci_free_statement($parsed);
		
				
    $monthSql = "SELECT MAX(MONTH) as MONTH FROM DESCO.MOD_SUMMARY_TOTAL WHERE YEAR = $year";
		$parsed = oci_parse($conn,$monthSql);
		oci_execute($parsed);
    $resultMonth = oci_fetch_array($parsed, OCI_BOTH + OCI_RETURN_NULLS);  
    $month = $resultMonth['MONTH'];	
		oci_free_statement($parsed);
		

		$rowSql = "SELECT COUNT(*) AS NSDD FROM DESCO.MOD_SUMMARY_TOTAL where YEAR = $year AND MONTH = $month";
		$parsed = oci_parse($conn,$rowSql);
		oci_execute($parsed);
		$resultRowCount = oci_fetch_array($parsed, OCI_BOTH + OCI_RETURN_NULLS);
    $sddNos = $resultRowCount['NSDD'];
		oci_free_statement($parsed);
				...
				<table class="table table-bordered border-secondary table-hover text-center align-middle">
						<thead>
							<tr>
								...
								<th scope="col" colspan='4'>YTD Sales and System Loss</th>
							...
							<tr>
								...
								<th scope="col">Monthly Collection</th>
								<th scope="col">Collection Adjustment</th>
							</tr>
						</thead>
						<tbody>						
							<form id='modForm' method='POST' action='save_mod.php'>
								<tr>
							...
							<?php
								...
								$descoRow =
										"<tr>
											<th scope='row'>" . $result['DIVISION'] . "</th>
											<th>" . number_format($result['IMPORT_UNIT'],0,".","") . "</th>
											<th><input type='number' step='1' id='' name='import9999' min='1' onChange='auditImportChng(this.parentElement.parentElement)'
												value='".number_format($result['IMPORT_UNIT'],0,".","")."' class='form-control'>
											</th>
											<th>" . number_format($result['SALES_UNIT'],0,".","") . "</th>
											<th>" . number_format($result['SYSTEM_LOSS'],2,".","") . "</th>
											<th><input type='number' step='1' onChange='adjustSales(this.parentElement.parentElement)' name='slsadjst9999' class='form-control'></th>											
											<th><input type='number' step ='0.0000001' onChange='adjustSystemLoss(this.parentElement.parentElement)' name='syslos9999' class='form-control'
														value='".number_format($result['SYSTEM_LOSS'],2,".","")."'>
											</th>
											...
											<th class='d-none'><input name='ytdimport9999' readonly type='number'
												value='" . number_format(($result['IMPORT_UNIT'] + $pblsdYtdPrevMnth['ytd_import_unit']),0,".","") . "'>												
											</th>
											...
											<th class='d-none'><input name='ytdsales9999' readonly type='number'
												value='" . number_format(($result['SALES_UNIT'] + $pblsdYtdPrevMnth['ytd_sales_unit']),0,".","") . "'>												
											</th>
											...
											<th>" . number_format(calculateSystemLoss($result['IMPORT_UNIT'] + $pblsdYtdPrevMnth['ytd_import_unit'],$result['SALES_UNIT'] + $pblsdYtdPrevMnth['ytd_sales_unit']),2,".","") . "</th>
											<th><input name='ytdsysloss9999' readonly type='number' class='form-control'
												value='" . number_format(calculateSystemLoss($result['IMPORT_UNIT'] + $pblsdYtdPrevMnth['ytd_import_unit'],$result['SALES_UNIT'] + $pblsdYtdPrevMnth['ytd_sales_unit']),2,".","") . "'>
											</th>
											<th>". number_format($ytdSlsAdjst['ytd_sales_adjustment'],0,".","") ."</th>
											<th>" . number_format($result['COLLECTION_AMOUNT'],0,".","") . "</th>
											<th><input type='number' step='1' name='coladj9999' onChange='adjustCollection(this.parentElement.parentElement)'
														value='' class='form-control'>
											</th>
											<th class='d-none'><input name='prvmnthytdbill9999' value='" . number_format($pblsdYtdPrevMnth['ytd_bill_amount'],0,".","") ."' type='number' readonly></th>
								...
								foreach($result as $dataRow){
									...
									$divisionRow =
											"<tr>												
												...
												<td><input class='form-control' name='import".$divisionId."'  onChange='auditImportChng(this.parentElement.parentElement)'
												...
												<td><input class='form-control' type='number' step='1' id='' name='slsadjst".$divisionId."'
													onChange='adjustSales(this.parentElement.parentElement)'>
												</td> 
												<td><input name='syslos".$divisionId."' value='".number_format($dataRow['SYSTEM_LOSS'],2,".","")."' type='number' readonly class='form-control'></td>																					
												...
												<td><input name='ytdsysloss".$divisionId."' type='number' readonly class='form-control'
												...
												<td><input class='form-control' name='coladj".$divisionId."' type='number' step='1' value=''
												  onChange='adjustCollection(this.parentElement.parentElement)' >
												</td>
												<td class='d-none'><input name='prvmnthytdbill".$divisionId."' value='" . number_format($pblsdYtdPrevMnth['ytd_bill_amount'],0,".","") ."' type='number' readonly ></td>
	...
	<?php 
			...
			function modPullDESCO($year,$month,$connection){
				$sql = "select 'DESCO' AS DIVISION, SUM(import_unit) AS import_unit,SUM(sales_unit) AS sales_unit,
								((SUM(import_unit)-SUM(sales_unit))/SUM(import_unit))*100 AS system_loss, SUM(COLLECTION_AMOUNT) AS COLLECTION_AMOUNT 
								from DESCO.mod_summary_total where year = '$year' and month = '$month'";				
			...
			function findYtdSalesAdjustment($division, $year, $month, $connection){
				...
				if($month <= 0){
					$month = 12;
					$year -= 1;
					$sql = "SELECT `ytd_sales_adjustment` FROM desco.ACTUAL_MOD WHERE `year`= $year AND `month` = $month AND division_id = '$divisionId'";
				}
				else if($month == 6){
					$sql = "SELECT 0 AS `ytd_sales_adjustment` FROM desco.ACTUAL_MOD WHERE `year`= $year AND `month` = $month AND division_id = '$divisionId'";
				}
				else{
					$sql = "SELECT `ytd_sales_adjustment` FROM desco.ACTUAL_MOD WHERE `year`= $year AND `month` = $month AND division_id = '$divisionId'";
				}
			...
			function ytdPblshdDataPull($division, $year, $month, $connection){
				...
				if($month <= 0){
					$month = 12;
					$year -= 1;
					$sql = "SELECT ytd_import_unit, ytd_sales_unit, ytd_bill_amount, ytd_coll_amount FROM desco.PUBLISHED_MOD WHERE `year`= $year AND `month` = $month AND division_id = '$divisionId'";
				}
				else if($month == 6){
					$sql = "SELECT 0 AS ytd_import_unit,  0 AS ytd_sales_unit, 0 AS ytd_bill_amount, 0 AS ytd_coll_amount FROM desco.PUBLISHED_MOD WHERE `year`= $year AND `month` = $month AND division_id = '$divisionId'";
				}
				else {
					$sql = "SELECT ytd_import_unit, ytd_sales_unit, ytd_bill_amount, ytd_coll_amount FROM desco.PUBLISHED_MOD WHERE `year`= $year AND `month` = $month AND division_id = '$divisionId'";
				}				
